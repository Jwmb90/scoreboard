...
from scraper import get_leaderboard_mapping_cached, force_refresh_leaderboard, CACHE_DURATION, _last_scrape
...
def generate_scoreboard():
    ...
    The total is formatted so that:
      - 0 is displayed as "E"
      - A positive total has a "+" sign
      - Negative totals are shown as-is.
    The list is sorted in ascending order based on the numeric total.
    """
    ...
    # Sort the list: lowest numeric total first
    scoreboard.sort(key=lambda x: x["total_numeric"])
    return scoreboard

def get_full_masters_scoreboard():
    """Returns all MasterScore records sorted by their numeric value (with 'E' treated as 0)."""
    ...
    sorted_scores = sorted(all_scores, key=lambda rec: convert_score(rec.current_score))
...
def index():
    ...
    # Use the cached leaderboard mapping for available golfers.
...
def edit_competitor(competitor_id):
    ...
    # Get available golfers from the cached leaderboard
    leaderboard_mapping = get_leaderboard_mapping_cached()
    available_golfers = sorted(leaderboard_mapping.keys())
...
def refresh():
    """
    Refresh the full Masters scoreboard by scraping ESPN;
    update MasterScore records and log history for changes.
    """
...
@app.route("/api/full")
def api_full():
    # Import _last_scrape and CACHE_DURATION from scraper.py to check if a refresh is needed.
    from scraper import _last_scrape, CACHE_DURATION
    if time.time() - _last_scrape > CACHE_DURATION:
